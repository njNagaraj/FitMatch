...
export const EventForm: React.FC<EventFormProps> = ({ isOpen, onClose, onSave, eventToEdit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" aria-modal="true" role="dialog">
      <div className="bg-light-bg-secondary dark:bg-dark-bg-secondary w-full max-w-lg max-h-[90vh] flex flex-col border border-light-border dark:border-dark-border rounded-lg shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="flex-grow p-6 space-y-4 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormRow label="Sport" error={errors.sport}>
                    <input type="text" value={sport} onChange={e => setSport(e.target.value)} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md focus:ring-primary focus:border-primary" />
                </FormRow>
                <FormRow label="City" error={errors.city}>
                    <input type="text" value={city} onChange={e => setCity(e.target.value)} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md focus:ring-primary focus:border-primary" />
                </FormRow>
            </div>
            
            <FormRow label="Date and Time" error={errors.date}>
                <input type="datetime-local" value={date} onChange={e => setDate(e.target.value)} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md focus:ring-primary focus:border-primary" />
            </FormRow>

            <FormRow label="Description" error={errors.description}>
                <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md focus:ring-primary focus:border-primary"></textarea>
            </FormRow>

            <FormRow label="Image URL" error={errors.imageUrl}>
                <input type="url" value={imageUrl} onChange={e => setImageUrl(e.target.value)} placeholder="https://example.com/image.jpg" className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md focus:ring-primary focus:border-primary" />
            </FormRow>
