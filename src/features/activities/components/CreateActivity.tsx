...
export const CreateActivity: React.FC<CreateActivityProps> = ({ setCurrentPage }) => {
  ...
  const { sports, createActivity } = useActivities();
      ...
      <div className="p-4 sm:p-8 h-full overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl bg-light-bg-secondary dark:bg-dark-bg-secondary p-6 border border-light-border dark:border-dark-border rounded-lg shadow-sm space-y-4">
          ...
          <FormRow label="Sport" error={errors.sportId}>
              <select value={sportId} onChange={handleSportChange} className="w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md focus:ring-primary focus:border-primary">
          ...
          <div className="pt-4">
              <button type="submit" className="w-full bg-primary hover:bg-primary-dark text-white font-semibold px-4 py-3 flex items-center justify-center gap-2 transition-colors rounded-md">
                  Create Activity
              </button>
          </div>
        </form>
