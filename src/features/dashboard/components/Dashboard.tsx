...
export const Dashboard: React.FC<DashboardProps> = ({ setCurrentPage }) => {
  ...
  const filteredActivities = useMemo(() => {
    return nearbyActivities.filter(activity => {
        const sport: Sport | undefined = getSportById(activity.sportId);
        const sportName = activity.otherSportName || sport?.name || '';
        const searchMatch = activity.title.toLowerCase().includes(searchTerm.toLowerCase()) || sportName.toLowerCase().includes(searchTerm.toLowerCase());
